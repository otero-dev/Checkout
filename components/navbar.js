...
const Nav = (props) => {
  ...
  if (props.metadata.length > 0) {
    props.metadata.map((field) => {
      if (field.key === "app_logo")
        tmp_store_logo = getValueFromObjectString(field.value, "src");
      if (field.key === "ssl_image")
        tmp_ssl_logo = getValueFromObjectString(field.value, "src");
      if (field.key === "contact_us_text")
        tmp_contact_text = getValueFromString(field.value, "html");
      if (field.key === "contact_us_email")
        tmp_contact_email = getValueFromString(field.value, "html");
    });
  }
