...
const OrderBump = (props) => {
  ...
  if (props.metadata.length > 0) {
    props.metadata.map((field) => {
      if (field.key === "order_bump_icon")
        tmp_bump_icon = getValueFromObjectString(field.value, "src");
      if (field.key === "order_bump_headline")
        tmp_bump_headline = getValueFromString(field.value, "html");
      if (field.key === "order_bump_offer")
        tmp_bump_offer = getValueFromString(field.value, "html");
    });
  }
  ...
  return (
    <BumpCard onClick={() => props.checkBump(props.active)}>
      ...
      <Box
        dangerouslySetInnerHTML={{ __html: bump_offer }}
        textAlign="center"
        px={10}
      />
    </BumpCard>
