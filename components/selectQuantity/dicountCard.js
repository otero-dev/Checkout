...
const DiscoundCard = (props) => {
    return (
        <DiscountMenuItem onClick={() => props.selectOrder(props.index)} style={{background: `${props.index===2 ? '#ffbd03' : 'white'}`}}>
            ...
            <Box ml={10} display='flex' alignItems='center' justifyContent='space-between' width="100%">
                <Box width="55%">
                    <Box>{props.index===2?<strong>Best Seller</strong>:''}&nbsp;{props.discount}% OFF: &nbsp;{props.quantity}&nbsp;{props.shortName}{props.quantity>1?'s': ''}</Box>                    
